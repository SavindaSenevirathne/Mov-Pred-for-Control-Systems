...
# To add unknown person to dataset
def addUnTodataSet(faceCapture):
    todayTime = datetime.datetime.now().strftime("%Y%b%d_%H%M%S")
    cv2.namedWindow(todayTime)
    img_counter = 0
    parent_dir = "opencvFaceRecognition/dataset/"
    directory = todayTime
    path = os.path.join(parent_dir, directory)
    os.mkdir(path)
    print("Press Space to take an image...")
    print("Press Escape to quite...")
    print("You shoud take minimum 5 images...")
    ...
    while True:
        ...
        if k % 256 == 27:
            ...
            if(img_counter >= 5):
                ...
                print(
                    "You should take minimum 5 images, Press Space to take an image...")
        elif k % 256 == 32:
            # SPACE pressed
            img_name = "{}.png".format(img_counter)
            cv2.imwrite(os.path.join(path, img_name), frame)
            print("{} written!".format(frame))
            img_counter += 1
    return todayTime
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if faceFrame is not None:
            ...
            if((height > 700) | (width > 1366)):
                faceFrame = cv2.resize(
                    faceFrame, (int(width/2), int(height/2)), interpolation=cv2.INTER_AREA)
        ...
        waitKey = cv2.waitKey(10)
        if waitKey == ord('x'):
                        # check recognized person is unknown or not
    ...
    while True:
        ...
        if frame is not None:
            ...
            if((height > 700) | (width > 1366)):
                frame = cv2.resize(
                    frame, (int(width/2), int(height/2)), interpolation=cv2.INTER_AREA)

        if not isObjectDetected:
            frameC, isObjectDetected, detectedObjects = detect(
        ...
        for i, newbox in enumerate(boxes):
            ...
            cv2.putText(frame, 'Tracking: ' + recognizedPerson, p1,
